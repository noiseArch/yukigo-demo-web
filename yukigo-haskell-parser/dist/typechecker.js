...
export class TypeChecker {
    ...
    check(ast) {
        ...
        traverse(ast, {
            function: (node) => {
                ...
                const functionType = this.signatureMap.get(functionName);
                if (!functionType)
                    this.errors.push(`Function '${functionName}' is used but not defined`);
                ...
                for (const func of node.contents) {
                    ...
                    const returnType = functionType.to;
                    // resolve param types
                    const paramTypes = functionType.from.map((t) => this.mapTypeNodePrimitives(t));
                    ...
                    if (isUnguardedBody(func)) {
                        ...
                        const funcInferredType = this.inferType(func.return.body, symbolMap, substitutions);
                        if (returnType.type === "ListType" &&
                        ...
                        subInferredType = this.applySubstitution(funcInferredType, substitutions);
                    }
                        ...
                        for (const guard of func.body) {
                            ...
                            const isOtherwise = guardBody.type === "YuSymbol" &&
                            ...
                            if (guardInferredType.type !== "TypeConstructor" ||
                                guardInferredType.name !== "YuBoolean") {
                                this.errors.push(`Guard condition must evaluate to a boolean (YuBoolean), but found ${this.formatType(guardInferredType)}`);
                                return;
                            }
                            const funcInferredType = this.inferType(guard.return.body, symbolMap, substitutions);
                            subReturnType = this.applySubstitution(returnType, substitutions);
                            subInferredType = this.applySubstitution(funcInferredType, substitutions);
                        }
                    }
                    if (!this.typeEquals(subReturnType, subInferredType)) {
                        this.errors.push(`Type mismatch in '${functionName}': Expected ${this.formatType(subReturnType)} but got ${this.formatType(subInferredType)}`);
                    }
                }
            },
        });
    ...
    buildGlobalEnvironment(ast) {
        traverse(ast, {
            TypeAlias: (node) => {
                const typeAliasIdentifier = node.name.value;
                if (this.typeAliasMap.has(typeAliasIdentifier)) {
                    this.errors.push(`Type alias '${typeAliasIdentifier}' is already defined`);
                    return;
                }
            ...
            Record: (node) => {
                ...
                for (const constructor of node.contents) {
                    const cons = Array.from(this.recordMap).some((cons1) => cons1[1].name === constructor.name);
                    if (cons) {
                        this.errors.push(`Constructor '${constructor.name}' is already defined`);
                        return;
                    }
                }
                ...
                try {
                    ...
                    for (const cons of node.contents) {
                        ...
                        resCons.fields.forEach((field) => field.name &&
            ...
            TypeSignature: (node) => {
                ...
                if (this.signatureMap.has(functionName)) {
                    this.errors.push(`Function '${functionName}' has multiple type signatures`);
                    return;
                }
                try {
                    const resolvedInputs = node.inputTypes.map((t) => this.mapTypeNodePrimitives(t));
                    const resolvedReturn = this.mapTypeNodePrimitives(returnType);
                    let finalInputs = resolvedInputs;
                    let finalReturn = resolvedReturn;
                    const isFunctionType = (returnType.type === "TypeVar" ||
    ...
    inferType(node, symbolMap, substitutions) {
        switch (node.type) {
            case "YuChar":
            case "YuString":
            case "YuNumber":
            ...
            case "YuList": {
                const elementInferredTypes = node.elements.map((element) => this.inferType(element.body, symbolMap, substitutions));
                const firstType = elementInferredTypes.length === 0
                    ? undefined
                ...
                const allElementsMatch = elementInferredTypes.every((element) => this.typeEquals(firstType, element));
                if (allElementsMatch) {
                    return {
                        type: "ListType",
                        element: firstType,
                ...
                this.errors.push(`List elements must be the same type. Found mixed types: ${this.formatType(firstType)} and others`);
                return {
                    type: "ListType",
            ...
            case "YuSymbol": {
                ...
                const symbolType = symbolMap.get(symbolValue) ??
            ...
            case "Arithmetic": {
                ...
                try {
                    ...
                    this.errors.push(`Arithmetic operation requires numbers, but got ${this.formatType(leftType)} and ${this.formatType(rightType)}`);
                    return { type: "TypeVar", name: "TypeError" };
                }
                return numberType;
            }
            case "CompositionExpression": {
                const leftHandType = this.inferType(node.left, symbolMap, substitutions);
                const rightHandType = this.inferType(node.right, symbolMap, substitutions);
                if (leftHandType.type !== "FunctionType") {
                    this.errors.push(`Left side of composition must be a function. Found: ${this.formatType(leftHandType)}`);
                    return { type: "TypeVar", name: "Error" };
                }
                if (rightHandType.type !== "FunctionType") {
                    this.errors.push(`Right side of composition must be a function. Found: ${this.formatType(rightHandType)}`);
                    return { type: "TypeVar", name: "Error" };
                }
                if (!this.typeEquals(leftHandType, rightHandType)) {
                    this.errors.push(`Function types in composition don't match: ${this.formatType(leftHandType)} vs ${this.formatType(rightHandType)}`);
                    return { type: "TypeVar", name: "Error" };
                }
                const leftReturn = leftHandType.to;
                if (leftReturn.type === "TypeConstructor" ||
                ...
                this.errors.push(`Cannot determine return type name for composition: ${JSON.stringify(leftReturn)}`);
                return { type: "TypeVar", name: "Error" };
            }
            case "IfThenElse": {
                const conditionType = this.inferType(node.condition.body, symbolMap, substitutions);
                const isConditionBoolean = conditionType.type == "TypeConstructor" &&
                    conditionType.name == "YuBoolean";
                if (!isConditionBoolean) {
                    this.errors.push(`Condition must be a boolean (YuBoolean), found ${this.formatType(conditionType)}`);
                    return { type: "TypeVar", name: "Error" };
                }
                const thenType = this.applySubstitution(this.inferType(node.then.body, symbolMap, substitutions), substitutions);
                const elseType = this.applySubstitution(this.inferType(node.else.body, symbolMap, substitutions), substitutions);
                if (!this.typeEquals(thenType, elseType)) {
                    this.errors.push(`If-then-else branches have different types: 'then' is ${this.formatType(thenType)} while 'else' is ${this.formatType(elseType)}`);
                    return { type: "TypeVar", name: "Error" };
                }
                return thenType;
            }
            case "Application": {
                const funcType = this.inferType(node.function.body, symbolMap, substitutions);
                const argType = this.inferType(node.parameter.type === "Expression"
                    ? node.parameter.body
                    : node.parameter, symbolMap, substitutions);
                // case for function with params
                if (funcType.type === "FunctionType" && funcType.from.length > 0) {
                    const expectedArgType = this.applySubstitution(funcType.from[0], substitutions);
                    try {
                        const newSub = this.unify(expectedArgType, argType);
                        newSub.forEach((value, key) => substitutions.set(key, value));
                    }
                    catch (error) {
                        this.errors.push(`Cannot apply ${this.formatType(argType)} to function expecting ${this.formatType(expectedArgType)}`);
                        return { type: "TypeVar", name: "Error" };
                    }
                    ...
                    try {
                        ...
                        this.errors.push(`Error while unifying ApplicationExpression: ${error.message}`);
                        return { type: "TypeVar", name: "Error" };
                    }
                }
            }
            ...
            case "DataExpression": {
                ...
                for (let i = 0; i < dataType.from.length; i++) {
                    ...
                    const inputType = this.inferType(inputValue, symbolMap, substitutions);
                    try {
                        ...
                        this.errors.push(`Field '${attributeName}' expected ${this.formatType(expectedType)} but got ${this.formatType(inputType)}`);
                        return { type: "TypeVar", name: "TypeError" };
                    }
                }
                return dataType.to;
            ...
            case "TupleExpression": {
                const elementTypes = node.elements.map((el) => this.inferType(el.body, symbolMap, substitutions));
            ...
            case "ConsExpression": {
                ...
                if (tailType.type !== "ListType") {
                    this.errors.push(`Right side of ':' must be a list. Found: ${this.formatType(tailType)}`);
                    return { type: "TypeVar", name: "TypeError" };
                }
                try {
                    ...
                    this.errors.push(`Cons operator ':' requires matching types. Head is ${this.formatType(headType)} but list contains ${this.formatType(tailType.element)}`);
                    return { type: "TypeVar", name: "TypeError" };
                }
            }
            case "Comparison": {
                ...
                try {
                    ...
                    this.errors.push(`Comparison requires matching types. Left is ${this.formatType(leftType)}, right is ${this.formatType(rightType)}`);
                    return { type: "TypeVar", name: "TypeError" };
                }
            ...
            case "Concat": {
                ...
                try {
                    ...
                    this.errors.push(`Concatenation requires both sides to be lists or strings. Got ${this.formatType(leftType)} and ${this.formatType(rightType)}`);
                    return { type: "TypeVar", name: "Error" };
                }
                const isYuString = unifiedType.type === "TypeConstructor" &&
                    unifiedType.name === "YuString";
                const isYuList = unifiedType.type === "ListType";
                if (isYuString || isYuList)
                    return unifiedType;
                ...
                try {
                    ...
                    try {
                        ...
                        this.errors.push(`Concat requires both sides to be strings or lists of the same type. ` +
                            `Got: ${JSON.stringify(leftType)} and ${JSON.stringify(rightType)}`);
                        return { type: "TypeVar", name: "Error" };
                    }
                }
            }
            case "LambdaExpression": {
                ...
                for (const param of node.parameters) {
                    if (param.type === "VariablePattern") {
                        ...
                        this.errors.push(`Unsupported pattern in lambda parameter: ${param.type}`);
                        return { type: "TypeVar", name: "Error" };
                    }
                }
                const bodyType = this.inferType(node.body.body, lambdaSymbolMap, substitutions);
                return {
                    type: "FunctionType",
                    from: paramTypes,
                    to: bodyType,
    ...
    unify(t1, t2) {
        ...
        if (t1.type === "FunctionType" && t2.type === "FunctionType") {
            ...
            const sub2 = this.unify(this.applySubstitution(t1.to, sub1), this.applySubstitution(t2.to, sub1));
            return new Map([...sub1, ...sub2]);
        }
        if (t1.type === "DataType" && t2.type === "DataType") {
            if (t1.name !== t2.name ||
                t1.constructors.length !== t2.constructors.length) {
                throw new Error(`Cannot unify ${this.formatType(t1)} with ${this.formatType(t2)}. Data types have different structures`);
            }
            let combinedSub = new Map();
            for (let i = 0; i < t1.constructors.length; i++) {
                for (let j = 0; j < t1.constructors[i].fields.length; j++) {
                    const f1 = this.applySubstitution(t1.constructors[i].fields[j], combinedSub);
                    const f2 = this.applySubstitution(t2.constructors[i].fields[j], combinedSub);
        ...
        if (t1.type === "TupleType" && t2.type === "TupleType") {
            if (t1.elements.length !== t2.elements.length) {
                throw new Error(`Tuple length mismatch: Expected ${t1.elements.length} elements but got ${t2.elements.length}`);
            }
            let combinedSub = new Map();
        ...
        throw new Error(`Type mismatch: Cannot unify ${this.formatType(t1)} with ${this.formatType(t2)}`);
    }
    unifyLists(t1, t2) {
        if (t1.length !== t2.length) {
            throw new Error(`Function arity mismatch: Expected ${t1.length} arguments but got ${t2.length}`);
        }
        return t1.reduce((sub, curr, i) => {
            const newSub = this.unify(this.applySubstitution(curr, sub), this.applySubstitution(t2[i], sub));
        ...
        switch (a.type) {
            ...
            case "FunctionType":
                if (b.type !== "FunctionType")
                    return false;
                return (this.typeEqualsList(a.from, b.from) && this.typeEquals(a.to, b.to));
            case "TypeApplication":
                if (b.type !== "TypeApplication")
                    return false;
                return (this.typeEquals(a.base, b.base) &&
            ...
            case "DataType":
                if (b.type !== "DataType")
                    return false;
                return (a.name === b.name &&
                    a.constructors.every((cons, i) => cons.fields.every((el, j) => this.typeEquals(el, b.constructors[i].fields[j]))));
            default:
                return false;
        }
    }
    ...
    resolveTypeAlias(type, visited = new Set()) {
        switch (type.type) {
            case "TypeConstructor":
            case "TypeVar": {
                ...
                if (this.typeAliasMap.has(type.name)) {
                    ...
                    return this.resolveTypeAlias(this.typeAliasMap.get(type.name), visited);
                }
                return type;
            }
            case "FunctionType":
                return {
                    type: "FunctionType",
                    from: type.from.map((t) => this.resolveTypeAlias(t, new Set(visited))),
            ...
            case "TupleType":
                return {
                    type: "TupleType",
                    elements: type.elements.map((el) => this.resolveTypeAlias(el, new Set(visited))),
                };
            case "DataType":
                return {
                    ...type,
                    constructors: type.constructors.flatMap((c) => ({
                        name: c.name,
                        fields: c.fields.map((f) => this.resolveTypeAlias(f, new Set(visited))),
                    })),
                };
            default:
                return type;
    ...
    bindVariable(name, type) {
        ...
        if (this.isTypeInfinite(name, type)) {
            throw new Error(`Infinite type detected: ${name} occurs in ${this.formatType(type)}`);
        }
        return new Map([[name, type]]);
    }
    isTypeInfinite(name, type) {
        switch (type.type) {
            case "TypeVar":
                return type.name === name;
            case "FunctionType":
                return (type.from.some((t) => this.isTypeInfinite(name, t)) ||
            ...
            case "DataType":
                return type.constructors.some((c) => c.fields.some((f) => this.isTypeInfinite(name, f)));
            case "TypeConstructor":
            case "TypeApplication":
                return false;
            default:
    ...
    resolvePatterns(param, symbolMap, paramTypes, i) {
        switch (param.type) {
            ...
            case "ConstructorPattern": {
                ...
                if (constructorType.type === "FunctionType")
                    param.patterns.forEach((el, j) => this.resolvePatterns(el, symbolMap, [...constructorType.from, constructorType.to], j));
            ...
            case "ListPattern": {
                ...
                if (listType.type === "ListType") {
                    param.elements.forEach((el) => this.resolvePatterns(el, symbolMap, [listType.element], 0));
                }
                else {
                    this.errors.push(`Pattern expects a list but found ${this.formatType(listType)}`);
                }
                break;
            }
            case "ConsPattern": {
                ...
                if (listType.type === "ListType") {
                    ...
                    this.errors.push(`Couldn't match expected type '${JSON.stringify(listType)}' with actual ListPattern`);
                }
                break;
            }
            case "AsPattern": {
            ...
            case "TuplePattern": {
                ...
                if (tupleParamType.type === "TupleType") {
                    if (Array.isArray(param.elements) &&
                        Array.isArray(tupleParamType.elements)) {
                        param.elements.forEach((pattern, idx) => {
                            this.resolvePatterns(pattern, symbolMap, tupleParamType.elements, idx);
                        });
                    }
                }
                else {
                    this.errors.push(`Pattern expects a tuple but found ${this.formatType(tupleParamType)}`);
                }
                break;
            }
            default:
    ...
    mapTypeNodePrimitives(type) {
        const mapPrimitiveNode = (t) => {
            switch (t.type) {
                case "TypeVar":
                case "TypeConstructor": {
    ...
    applySubstitution(type, sub) {
        const substituteNode = (t) => {
            if (t.type === "TypeVar") {
